...
void __interrupt() isr(void) {
    ...
    if(INT0IF == 1) {           // INT0外部割り込み発生か？
        {
            /* メッセージ出力 */
            uint8_t msg[] = ">ExtInt0\r\n";
            send_strg(msg);     // USART送信 (文字列)
        }
        INT0IF = 0;             // INT0外部割り込みフラグをクリアする
    }
    if(INT1IF == 1) {           // INT1外部割り込み発生か？
        {
            /* メッセージ出力 */
            uint8_t msg[] = ">ExtInt1\r\n";
            send_strg(msg);     // USART送信 (文字列)
        }
        INT1IF = 0;             // INT1外部割り込みフラグをクリアする
    }
}
...
void main(void) {
    ...
    TRISA   = 0b11101000;       // RA0-2,4のみ出力に設定、1で入力 0で出力
